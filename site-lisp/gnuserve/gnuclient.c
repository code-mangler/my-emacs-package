...
BOOL filename_expand ( char *fullpath, char *filename )
{
  ...
  if (!GetFullPathName (filename, MAXPATHLEN + 2, fullpath, &fname)) {
    sprintf (errbuf, "Unable to get full pathname for %s\n", filename);
    show_error(errbuf);
    return (FALSE);
  }
...
void showEmacs( int unIconify, int startFlag )
{
...
char *get_current_working_directory()
{
  ...
  */
  ...
  char *argv[];
  {
    ...
    int oflg = 0; 			/* use old names (server-* instead of gnuserv-*) */
    int errflg = 0;			/* option error */
    int c;				/* char from getopt */
    char getopt_errbuf[1024];           /* collect errors from getopt */
    ...
    strcpy(getopt_errbuf, "");
    while ((c = getopt_win(argc, argv,
		       ...
		       , getopt_errbuf)) != EOF)
      switch (c) {
      case 's':					/* start gnuserv */
	sflg++;
	break;
      case 'q':					/* quick mode specified */
	qflg++;
	break;
      case 'w':					/* wait mode specified */
	qflg = 0;
	break;
      case 'e':					/* eval sexpr */
	eflg++;
...
#ifdef INTERNET_DOMAIN_SOCKETS
#if defined( WIN32 )
	      "usage: %s [-qwfFno] [-h host] [-p port] [-r pathname] [[+line] path] ...\n"
	      "       or        [-qwfFo] [-h host] [-p port] -e [sexpr]\n"
	      "       or        -x\n\n"
	      "Notice that emacs.exe takes other arguments.",
#else
...
#endif /* !INTERNET_DOMAIN_SOCKETS */
	      progname);
      strcat(getopt_errbuf, errmsg);
      show_error(getopt_errbuf);
      exit (1);
    }; /* if */
...
#ifdef INTERNET_DOMAIN_SOCKETS
    connect_type = make_connection(hostarg, portarg, &s, sflg);
#else
    connect_type = make_connection(NULL, (u_short) 0, &s, sflg);
#endif
    ...
    if (eflg) {
      if (qflg) {
	if (oflg) { send_string(s,"(server-eval-quickly '(progn ");
	} else { send_string(s,"(gnuserv-eval-quickly '(progn "); }
      }
      else {
	if (oflg) { send_string(s,"(server-eval '(progn ");
	} else { send_string(s,"(gnuserv-eval '(progn "); }
      };
      if (optind < argc) {
	...
	while ((nb = read(fileno(stdin), buf, GSERV_BUFSZ-1)) > 0) {
      ...
      if (qflg) {
	if (oflg) { send_string(s,"(server-edit-files-quickly '(");
	} else { send_string(s,"(gnuserv-edit-files-quickly '("); }
      }
      else {
	if (oflg) { send_string(s,"(server-edit-files '(");
	} else { send_string(s,"(gnuserv-edit-files '("); }
      };
