...
void handle_response( int fd )
{
  ...
  if ((soc = accept(fd,(struct sockaddr *)&peer,&addrlen)) == -1) {
    show_wsa_error("unable to accept");
    exit(1);
  }; /* if */
  ...
  if ( result_len < 0 )
  {
    show_error("unable to read stdin");
    exit(1);
  }

  while (result_len > 0) {
    if ((len = recv(soc,buf,min2(result_len,GSERV_BUFSZ),0)) < 0) {
      show_wsa_error("unable to recv at point 2");
      exit(1);
    }
    buf[len] = '\0';
    send_string(s,buf);
     ...
     u_long host_addr;
{
  ...
  if (!permitted(host_addr)) {
    if ((new_entry = (struct entry *) malloc(sizeof(struct entry))) == NULL) {
      ...
      show_error("unable to malloc space for permitted host entry\n");
      exit(1);
    }; /* if */
...
int setup_table()
{
  ...
  if ((host_addr = internet_addr(get_local_addr(hostname))) == -1) {
    sprintf(errbuf,"unable to find %s in /etc/hosts or from YP", hostname);
    show_error(errbuf);
    exit(1);
  }; /* if */
...
int internet_init()
{
  ...
  if ((ls = socket (AF_INET,SOCK_STREAM, 0)) == -1) {
    show_net_error("unable to create socket");
    exit(1);
  }; /* if */
  
  /* Bind the listen address to the socket. */
  if (bind(ls,(struct sockaddr *) &server,sizeof(struct sockaddr_in)) == -1) {
    show_net_error("unable to bind socket");
    exit(1);
  }; /* if */
   ...
   */
  if (listen(ls,20) == -1) {
    show_net_error("unable to listen");
    exit(1);
  }; /* if */
...
/* rather like connect_to_internet_server() except we make sure that
 * we don't use any environment variables to configure host or port,
 * and make sure that we don't try to run emacs from the server
 */
     ...
     int *s;
{
  ...
  if ((peeraddr_in.sin_addr.s_addr = internet_addr(get_local_addr(localhost))) == -1) {
    sprintf(errbuf,"unable to find %s in /etc/hosts or from YP\n", localhost);
    show_error(errbuf);
    exit(1);
  }
  ...
  if ((*s = socket (AF_INET,SOCK_STREAM, 0))== -1) {
    show_wsa_error("unable to create socket");
    exit(1);
  }
  ...
  if (connect(*s, (struct sockaddr *)&peeraddr_in,
    ...
    show_wsa_error("unable to connect to client");
    exit(1);
  }
}
...
/*
  WIN32
  win_port_init -- Create an internet port that we can listen on, a separate thread will
...
int win_port_init()
{
  ...
  /* Bind the listen address to the socket. */
  if (bind(ls,(struct sockaddr *) &server,sizeof(struct sockaddr_in)) == -1) {
    show_wsa_error("unable to bind socket");
    exit(1);
  }; /* if */
   ...
   */
  if (listen(ls,20) == -1) {
    show_wsa_error("unable to listen");
    exit(1);
  }; /* if */
...
int ls;				/* listen socket */
{
  ...
  if ((s = accept(ls,(struct sockaddr *)&peer,&addrlen)) == -1) {
    show_net_error("unable to accept");
    exit(1);
  }; /* if */
     ...
     char *argv[];
{
  ...
  progname = argv[0];
  is_win_version = 1;  // Popup error message with MessageBox
  ...
  while (1) {
    ...
    if (select(max2(wls,max2(fileno(stdin),max2(uls,ils))) + 1, &rmask, 
      ...
      {
	show_net_error("unable to select");
	exit(1);
      }; /* if */
