...
extern void show_wsa_error(char* sMsg);
extern void show_net_error(char* sMsg);
//extern void setForegroundWindow(HWND i_hwnd);
extern void win_show_emacs(void);
...
#define CONN_IPC      2
 ...
 int connect_to_internet_server( char *serverhost, int port, int startFlag );
 int make_connection( char *hostarg, int portarg, int *s, int startFlag );
 void send_string( int s, char *msg );
 void disconnect_from_server( int s, int echo );
 int internet_addr( char *host );
 char * get_local_addr( char * addr );
 void show_error(char * errmsg);
 void show_error_no(char * errmsg, int err_no);

 #if defined( WIN32 )
  void WinsockInit(void);
  BOOL StartEmacs(int startFlag, BOOL bWait);
  #include <windows.h>
  #include <ctype.h>
 #endif
#endif
